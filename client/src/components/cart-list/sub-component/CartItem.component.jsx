...
const CartItem = ({
  ...
  const { name, image, sku } = product;
  const { size, color, quantity, colorPrice, colorDiscount } = order;
      ...
      <div className="cart-item row d-flex flex-wrap mx-2 my-4 pt-3 pb-4">
        ...
        <div className="detail pl-4 d-flex flex-column">
          ...
          <div className="price">
            {colorDiscount === 0 ? (
              <>
                <span className="mr-3">${colorPrice.toFixed(2)}</span>
...
const mapStateToProps = createStructuredSelector({
  cartItems: selectCartItems, // must put it here to force CardItem getting updated after changing quantity
});
