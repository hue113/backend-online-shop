...
const RelatedProductCard = ({ product, addItemToFavourite }) => {
  ...
  const { name, image, sku, price, isNewItem, variation } = product;

  return (
    <>
      <div className="product-card-grid">
        <div className="image-box mb-3">
          <a href={`/#/products/${name.toLowerCase().replace(/ /g, '-')}.${sku}`}>
          ...
          {variation[0].discount === 0 ? (
            ...
            <div className="special sale p-3">-{variation[0].discount}%</div>
          )}
          ...
          {variation[0].discount === 0 ? (
            ...
            <div>
              ...
              <span className="sale-price">
                ${calculatePrice(price, variation[0].discount)}
              </span>
            </div>
          )}
        </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(RelatedProductCard);
