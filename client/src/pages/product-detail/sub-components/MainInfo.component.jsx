...
const MainInfo = ({ product, param, addItemToFavourite, addItemToCart }) => {
  const { name, variation, shortDescription } = product;

  let saleColor;
  let saleItem;
  if (param.split('---')[1]) {
    saleColor = param.split('---')[1].split('.')[0];
    saleItem = variation.find((i) => i.color === saleColor);
  }

  const [selectVariation, setVariation] = useState({
    color: saleColor ? saleColor : variation[0].color,
    colorPrice: saleColor ? saleItem.price : variation[0].price,
    colorDiscount: saleColor ? saleItem.discount : variation[0].discount,
  });
